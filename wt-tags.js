...
const WT_DESCRIPTIONS = {
  ...
  pistol: `
    <b>PISTOL</b><br>
    in their profile are known as <span class="kwb2 bluefont">[PISTOL]</span>. If a unit contains any models equipped with Pistols, that unit is eligible to shoot in its controlling player’s 
	...
	before selecting targets.<br/><br/><ul class="BreakInsideAvoid frameLight Corner14"><li>Can be shot even if the bearer’s unit is within
  ...
  `,
    ...
    <b>BLIND</b><br>
	<br>
    Attack looses a brilliant flare of light, searing the sight of the victim and forcing them to fight blind for a few moments.
	<br>
	<ul class="BreakInsideAvoid frameLight Corner14"><li>
	If your unit hits 1 or more enemy units with a weapon with <span class="kwb2 bluefont">[BLIND X]</span>. You can only target 1 enemy unit, and it must pass 
...
if (this.innerText.includes(label)) {
  this.innerText = this.innerText.replace(label, "");
} else {
...
class WeaponTable extends HTMLElement {
  connectedCallback() {
	...
	if (type == `RANGED WEAPONS`){
		
		title.innerHTML = `<span>🔫</span> ${type}`;
	}
	if (type == `MELEE WEAPONS`){
		
		title.innerHTML = `<span>⚔️</span> ${type}`;
	}
	if (type != `MELEE WEAPONS` && type != `RANGED WEAPONS`){
	...
	if (type == `RANGED WEAPONS`){
		...
		const fields = ["R", "A", "BS", "S", "AP", "D"];
		for (let f of fields) {
        const td = document.createElement("td");
        td.textContent = weapon.getAttribute(f) || "-";
        tr.appendChild(td);
      }
	}
	if (type == `MELEE WEAPONS`){
		
		const fields = ["R", "A", "WS", "S", "AP", "D"];
		for (let f of fields) {
        const td = document.createElement("td");
        td.textContent = weapon.getAttribute(f) || "-";
        tr.appendChild(td);
...
// Регистрируем новый кастомный элемент
